...
function reduceReceiveSamplesList(state, action) {
    ...
    const sortedSamples = _.sortBy(samples, (sample) => sample.fileName.toLowerCase()); // TODO 757 fileName
...
function addSamples(state, action) {
    ...
    const sortedSamples = _.sortBy(newSampleList, (sample) => sample.fileName.toLowerCase()); // TODO 757 fileName
    return {
        ...state,
        hashedArray: ImmutableHashedArray.makeFromArray(sortedSamples)
    };
